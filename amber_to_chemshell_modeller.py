...
def crop_topology(args):
    ...
    topology.write_parm(args.output + '.cropped.prmtop')
    topology.save(args.output + '.cropped.inpcrd')
    #for res in topology.residues:
    #   res.ter = False
    topology.write_pdb(args.output + '.cropped.pdb')

    pdb = open(args.output + '.cropped.pdb', 'r').readlines()
    pdb_o = open(args.output + '.cropped.pdb.tmp', 'w')
    ...
    pdb_o.close()
    os.remove(args.output + '.cropped.pdb')
    os.rename(args.output + '.cropped.pdb.tmp', args.output + '.cropped.pdb')
